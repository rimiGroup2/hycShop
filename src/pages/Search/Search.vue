  ...
  <div class="search">
    ...
    <header class="header-box">
      <Row :gutter="6">
          ...
          <Col span="17">
            <Input v-model="searchInput" :autofocus="true" prefix="ios-search" placeholder="双十一狂欢" width="76%;"/>
          </Col>
          <Col span="4">
    ...
    <div class="search-content">
      ...
      <div class="history-list">
        ...
        <div class="top-data" style="text-align:left;">
          <span v-for="obj in topData" :key="obj.id" @click="searchIcon(obj)">{{obj}}</span>
        </div>
      </div> 
      <!-- 热门搜索 -->
      <div class="hot-list">
        <p>热门搜索</p>
        <span v-for="(obj,index) in hotList" :key="obj.id" @click="searchIcon(obj)">{{obj}}</span>
      </div>
    </div>   
  </div>
</template>
  ...
  methods:{
    ...
    searchData(){
      if(this.searchInput==""){
        ...
        this.getHistoryList();
        this.$router.push({path:'/productlist',query:{search:this.searchInput}})
        //清空输入框
        this.searchInput="";  
      }
    },
    ...
    searchIcon(obj){
      this.$router.push({path:'/productlist',query:{search:obj}})
    }
  },mounted(){
